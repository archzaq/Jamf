...
function password_Prompt(){
    ...
    password=$(/usr/bin/osascript <<OOP
        set password to (display dialog "Please enter the password you would like to use for your elevated account:" buttons {"Cancel", "OK"} default button "OK" with hidden answer default answer "" with icon POSIX file "$iconPath" with title "$passwordTitle" giving up after 900)
        if button returned of password is equal to "OK" then
            return text returned of password
        else
            return "timeout"
        end if
OOP
...
function main() {
    ...
    if ! password_Prompt "elevatedAccountPass";
    then
